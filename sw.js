...
self.addEventListener('fetch',function(e){e.respondWith(caches.match(e.request).then(function(r){return r||fetch(e.request).then(function(nr){if(nr.ok){var c=nr.clone();caches.open(CACHE).then(function(ca){ca.put(e.request,c)})}return nr}).catch(function(){return new Response('Offline — open a previously cached book.',{headers:{'Content-Type':'text/html'}})})}))}); 